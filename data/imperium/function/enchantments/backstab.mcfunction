...
# Candidates = humans in front of the wielder within melee reach (a point 2 blocks ahead along his
# flattened facing, radius 2.5). For each, backstab_behind arms only if he's also behind it. The
# behind-test is restrictive, so an over-generous front zone is fine (extras get filtered there).
execute at @s anchored eyes rotated ~ 0 positioned ^ ^ ^2.0 as @e[type=#imperium:human,tag=!im.backstab_self,distance=..2.5] run function imperium:enchantments/backstab_behind
