...
# Re-equip their kit so a death can't strand them itemless (givekit runs clear_kit first,
# so this is safe even if the server keeps inventories on death).
execute if score @s im_lastKit matches 1..7 run scoreboard players operation @s givekit = @s im_lastKit
