...
# subtracts), so future gold still accrues normally afterward. #lost im.temp holds the amount lost
# (kept through the kill-feed handoff below). When im_lives hits 0 the fighter is eliminated (arena/eliminate).
...
scoreboard players operation #lost im.temp = @s im_gold
scoreboard players operation #lost im.temp /= #2 imperium.const
scoreboard players operation @s im_goldLost += #lost im.temp
function imperium:arena/calc_gold
...
# recorded attacker id (#atk) before switching context to the killer (#lost already holds the forfeit).
...
execute if score @s im_lives matches 1..2 run tellraw @s [{"text":" -","color":"red"},{"score":{"name":"#lost","objective":"im.temp"},"color":"gold"},{"text":" gold","color":"red"},{"text":" | Lives remaining: ","color":"red"},{"score":{"name":"@s","objective":"im_lives"},"color":"yellow"}]
