...
summon block_display ~ ~ ~ {Tags:["im.web","im.web_new","summit.booth_entity.imperium"],block_state:{Name:"minecraft:cobweb"},transformation:{translation:[-0.5f,0f,-0.5f],scale:[1f,1f,1f],left_rotation:[0f,0f,0f,1f],right_rotation:[0f,0f,0f,1f]}}

# Seed the new web's lifetime (#WebDuration ticks); the loop ticks im_webLife down and kills it at 0.
scoreboard players operation @e[type=block_display,tag=im.web_new] im_webLife = #WebDuration im.param
tag @e[type=block_display,tag=im.web_new] remove im.web_new
